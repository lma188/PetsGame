...
public class Cat implements Animal{
	...
	private static String favToy = "Twine";
	private static String favFood = "Pizza";
	private static int damage = 15;
	private static String speciesName = "Cat";
	private static int hungerCo = 15;
	private static int tiredCo = 40;
	private static int playCo = 30;
	private static int toiletCo = 10;
	private static float originalWeight = (float) 5;
	private String name;
...
//	super("Twine", "Pizza", 15, "Cat", 15, 40, 30, 10, (float) 5, catName, "Cat");
//}
	...
	public static void printSpeciesStats(){
		System.out.println(String.format("Species: %s", Cat.getSpeciesName()));
		System.out.println(String.format("Favourite Toy: %s", Cat.getFavToy()));
		System.out.println(String.format("Favourite Food: %s", Cat.getFavFood()));
		System.out.println(String.format("Damage done to toy per use: %d points of toy's quality (starts at 100)", Cat.getDamage()));
		System.out.println(String.format("Hunger increase per day: %d points of hunger, where hunger is a rating of 0-100", Cat.getHungerCo()));
		System.out.println(String.format("Tiredness increase per day: %d points of tiredness, where tiredness is a rating of 0-100", Cat.getTiredCo()));
		System.out.println(String.format("Playfulness increase per day: %d points of playfulness, where playfulness is a rating of 0-100", Cat.getPlayCo()));
		System.out.println(String.format("Toilet need increase per day: %d points of toilet need, where toilet need is a rating of 0-100", Cat.getToiletCo()));
		System.out.println(String.format("Starting weight: %.2f kg", Cat.getOriginalWeight()));
		System.out.println();
	}
	...
	public void feed(pets2.Food food){
		...
		if(Cat.getFavFood() == Food.getName()){
	...
	public boolean play(String toyName, Player player){
		...
		if(toy.getQuality() - Cat.getDamage() < 0 || this.getIsMisbehaving()){
			toy.setQuality(0);
		}else{
			toy.setQuality(toy.getQuality() - Cat.getDamage());
		}
		if(toy.getQuality() <= 0){
	...
	public void dailyUpdateStats(){
		if(this.getIsAlive() == true){
			if(this.getHungerLevel() + Cat.getHungerCo() >= 100){
				this.setHungerLevel(100);
			}else{
				this.setHungerLevel(this.getHungerLevel() + Cat.getHungerCo());
			}
			if(this.getTiredLevel() + Cat.getTiredCo() >= 100){
				this.setTiredLevel(100);
			}else{
				this.setTiredLevel(this.getTiredLevel() + Cat.getTiredCo());
			}
			if(this.getPlayfulLevel() + Cat.getPlayCo() >= 100){
				this.setPlayfulLevel(100);
			}else{
				this.setPlayfulLevel(this.getPlayfulLevel() + Cat.getPlayCo());
			}
			if(this.getToiletLevel() + Cat.getToiletCo() >= 100){
				this.setToiletLevel(100);
			}else{
				this.setToiletLevel(this.getToiletLevel() + Cat.getToiletCo());
			}
			this.setActions(0);

...
public class Bird implements Animal{
	...
	private static String favToy = "Bell";
	private static String favFood = "Kale";
	private static int damage = 10;
	private static String speciesName = "Bird";
	private static int hungerCo = 20;
	private static int tiredCo = 10;
	private static int playCo = 10;
	private static int toiletCo = 50;
	private static float originalWeight = (float) 1;
	private String name;
...
//	super("Bell", "Kale", 10, "Bird", 20, 10, 10, 50, (float) 1, birdName, "Bird");
//}
	...
	public static void printSpeciesStats(){
		System.out.println(String.format("Species: %s", Bird.getSpeciesName()));
		System.out.println(String.format("Favourite Toy: %s", Bird.getFavToy()));
		System.out.println(String.format("Favourite Food: %s", Bird.getFavFood()));
		System.out.println(String.format("Damage done to toy per use: %d points of toy's quality (starts at 100)", Bird.getDamage()));
		System.out.println(String.format("Hunger increase per day: %d points of hunger, where hunger is a rating of 0-100", Bird.getHungerCo()));
		System.out.println(String.format("Tiredness increase per day: %d points of tiredness, where tiredness is a rating of 0-100", Bird.getTiredCo()));
		System.out.println(String.format("Playfulness increase per day: %d points of playfulness, where playfulness is a rating of 0-100", Bird.getPlayCo()));
		System.out.println(String.format("Toilet need increase per day: %d points of toilet need, where toilet need is a rating of 0-100", Bird.getToiletCo()));
		System.out.println(String.format("Starting weight: %.2f kg", Bird.getOriginalWeight()));
		System.out.println();
	}
	...
	public void feed(pets2.Food food){
		...
		if(Bird.getFavFood() == food.getName()){
	...
	public boolean play(String toyName, Player player){
		...
		if(toy.getQuality() - Bird.getDamage() < 0 || this.getIsMisbehaving()){
			toy.setQuality(0);
		}else{
			toy.setQuality(toy.getQuality() - Bird.getDamage());
		}
		if(toy.getQuality() <= 0){
	...
	public void dailyUpdateStats(){
		if(this.getIsAlive() == true){
			if(this.getHungerLevel() + Bird.getHungerCo() >= 100){
				this.setHungerLevel(100);
			}else{
				this.setHungerLevel(this.getHungerLevel() + Bird.getHungerCo());
			}
			if(this.getTiredLevel() + Bird.getTiredCo() >= 100){
				this.setTiredLevel(100);
			}else{
				this.setTiredLevel(this.getTiredLevel() + Bird.getTiredCo());
			}
			if(this.getPlayfulLevel() + Bird.getPlayCo() >= 100){
				this.setPlayfulLevel(100);
			}else{
				this.setPlayfulLevel(this.getPlayfulLevel() + Bird.getPlayCo());
			}
			if(this.getToiletLevel() + Bird.getToiletCo() >= 100){
				this.setToiletLevel(100);
			}else{
				this.setToiletLevel(this.getToiletLevel() + Bird.getToiletCo());
			}
			this.setActions(0);

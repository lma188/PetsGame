...
public class Panda implements Animal{
	private static String favToy;
	private static String favFood;
	private static int damage;
	private static String speciesName;
	private static int hungerCo;
	private static int tiredCo;
	private static int playCo;
	private static int toiletCo;
	private static float originalWeight;
	private String name;
...
//		super("PlayHouse", "Spaghetti", 10, "Panda", 30, 20, 20, 20, (float) 70, pandaName, "Panda");
//	}
	...
	public static void printSpeciesStats(){
		System.out.println(String.format("Species: %s", Panda.getSpeciesName()));
		System.out.println(String.format("Favourite Toy: %s", Panda.getFavToy()));
		System.out.println(String.format("Favourite Food: %s", Panda.getFavFood()));
		System.out.println(String.format("Damage done to toy per use: %d points of toy's quality (starts at 100)", Panda.getDamage()));
		System.out.println(String.format("Hunger increase per day: %d points of hunger, where hunger is a rating of 0-100", Panda.getHungerCo()));
		System.out.println(String.format("Tiredness increase per day: %d points of tiredness, where tiredness is a rating of 0-100", Panda.getTiredCo()));
		System.out.println(String.format("Playfulness increase per day: %d points of playfulness, where playfulness is a rating of 0-100", Panda.getPlayCo()));
		System.out.println(String.format("Toilet need increase per day: %d points of toilet need, where toilet need is a rating of 0-100", Panda.getToiletCo()));
		System.out.println(String.format("Starting weight: %.2f kg", Panda.getOriginalWeight()));
		System.out.println();
	}
	...
	public boolean play(String toyName, Player player){
		...
		if(toy.getQuality() - Panda.getDamage() < 0 || this.getIsMisbehaving()){
			toy.setQuality(0);
		}else{
			toy.setQuality(toy.getQuality() - Panda.getDamage());
		}
		if(toy.getQuality() <= 0){
	...
	public void dailyUpdateStats(){
		if(this.getIsAlive() == true){
			if(this.getHungerLevel() + Panda.getHungerCo() >= 100){
				this.setHungerLevel(100);
			}else{
				this.setHungerLevel(this.getHungerLevel() + Panda.getHungerCo());
			}
			if(this.getTiredLevel() + Panda.getTiredCo() >= 100){
				this.setTiredLevel(100);
			}else{
				this.setTiredLevel(this.getTiredLevel() + Panda.getTiredCo());
			}
			if(this.getPlayfulLevel() + Panda.getPlayCo() >= 100){
				this.setPlayfulLevel(100);
			}else{
				this.setPlayfulLevel(this.getPlayfulLevel() + Panda.getPlayCo());
			}
			if(this.getToiletLevel() + Panda.getToiletCo() >= 100){
				this.setToiletLevel(100);
			}else{
				this.setToiletLevel(this.getToiletLevel() + Panda.getToiletCo());
			}
			this.setActions(0);

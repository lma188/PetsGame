...
public class Dog implements Animal{
	...
	private static String favFood = "Chocolate";
	private static int damage = 20;
	private static String speciesName = "Dog";
	private static int hungerCo = 30;
	private static int tiredCo = 10;
	private static int playCo = 40;
	private static int toiletCo = 30;
	private static float originalWeight = (float) 22;
	private String name;
...
//		super("Ball", "Chocolate", 20, "Dog", 30, 10, 40, 30, (float) 22, dogName, "Dog");
//	}
	...
	public static void printSpeciesStats(){
		System.out.println(String.format("Species: %s", Dog.getSpeciesName()));
		System.out.println(String.format("Favourite Toy: %s", Dog.getFavToy()));
		System.out.println(String.format("Favourite Food: %s", Dog.getFavFood()));
		System.out.println(String.format("Damage done to toy per use: %d points of toy's quality (starts at 100)", Dog.getDamage()));
		System.out.println(String.format("Hunger increase per day: %d points of hunger, where hunger is a rating of 0-100", Dog.getHungerCo()));
		System.out.println(String.format("Tiredness increase per day: %d points of tiredness, where tiredness is a rating of 0-100", Dog.getTiredCo()));
		System.out.println(String.format("Playfulness increase per day: %d points of playfulness, where playfulness is a rating of 0-100", Dog.getPlayCo()));
		System.out.println(String.format("Toilet need increase per day: %d points of toilet need, where toilet need is a rating of 0-100", Dog.getToiletCo()));
		System.out.println(String.format("Starting weight: %.2f kg", Dog.getOriginalWeight()));
		System.out.println();
	}
	...
	public boolean play(String toyName, Player player){
		...
		if(toy.getQuality() - Dog.getDamage() < 0 || this.getIsMisbehaving()){
			toy.setQuality(0);
		}else{
			toy.setQuality(toy.getQuality() - Dog.getDamage());
		}
		if(toy.getQuality() <= 0){
	...
	public void dailyUpdateStats(){
		if(this.getIsAlive() == true){
			if(this.getHungerLevel() + Dog.getHungerCo() >= 100){
				this.setHungerLevel(100);
			}else{
				this.setHungerLevel(this.getHungerLevel() + Dog.getHungerCo());
			}
			if(this.getTiredLevel() + Dog.getTiredCo() >= 100){
				this.setTiredLevel(100);
			}else{
				this.setTiredLevel(this.getTiredLevel() + Dog.getTiredCo());
			}
			if(this.getPlayfulLevel() + Dog.getPlayCo() >= 100){
				this.setPlayfulLevel(100);
			}else{
				this.setPlayfulLevel(this.getPlayfulLevel() + Dog.getPlayCo());
			}
			if(this.getToiletLevel() + Dog.getToiletCo() >= 100){
				this.setToiletLevel(100);
			}else{
				this.setToiletLevel(this.getToiletLevel() + Dog.getToiletCo());
			}
			this.setActions(0);
